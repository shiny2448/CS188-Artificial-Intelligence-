    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                probBlue = 0
                probNone = 0
                if assignment[FOOD_HOUSE_VAR]==getHouseVal(obsPos[0],obsPos[1]):
                    probRed = PROB_FOOD_RED
                    probBlue = 1 - probRed
                    probNone = 0
                elif assignment[GHOST_HOUSE_VAR]==getHouseVal(obsPos[0],obsPos[1]):
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        leftExpectedValue = CPT.getProbability(ftopleft)*WON_GAME_REWARD+CPT.getProbability(ftopright)*GHOST_COLLISION_REWARD
        rightExpectedValue = CPT.getProbability(ftopleft)*GHOST_COLLISION_REWARD+CPT.getProbability(ftopright)*WON_GAME_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for (prob,evidence0) in probsAndOutcomes:
            expectedValue += prob*max(self.computeEnterValues(evidence0,enterEliminationOrder))
