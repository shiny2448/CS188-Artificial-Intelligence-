...
class SigmoidNeuron(object):
    def __init__(self, numFeatures):
        ...
        #self.weights = []#{}
        #for i in range(2):
        #    self.weights[i] = [0 for _ in range(numFeatures)]
        #self.weights = [[0 for _ in range(numFeatures)] for _ in range(2)]
        #self.weights = [0 for _ in range(numFeatures)]
        self.weights = np.array([0 for _ in range(numFeatures)])
    ...
    def train_single(self, sample, alpha, label):
        ...
        grad = np.array(nu.gradient(lambda w: self.loss(sample,label,w),self.weights))
        self.weights = self.weights - (grad*alpha)
        #for index in range(self.numFeatures):
            #if self.classify(sample):
        #    self.weights[index] = self.weights[index] - (alpha * grad[index])
    ...
    def train(self, samples, alpha, labels):
        ...
        for i in range(samples.shape[0]):
            #print 'len of samples:',samples.shape
