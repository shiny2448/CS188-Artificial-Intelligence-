...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        "*** YOUR CODE HERE ***"
        total = self.total()
        if self.total() != 0 and len(self.keys()) != 0:
            for key in self.keys():
                prob = self.get(key)
                dict.__setitem__(self,key,float(prob)/float(total))
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        #    return 0
        if noisyDistance == None:
            if ghostPosition==jailPosition: #dont need this line. i know.
                return 1
            return 0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            likelihood = self.getObservationProb(observation, gameState.getPacmanPosition(), pos, self.getJailPosition())
            if likelihood > 0:
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, likelihood in newPosDist.items():
                new[newPos] += self.beliefs[oldPos]*likelihood
        self.beliefs = new
        #self.beliefs.normalize()
